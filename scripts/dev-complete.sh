...
if [[ $_comp_cword -eq 1 ]]; then
    compgen -W "new enter delete start see show cp cpout use list pull install" -- "$_comp_cur"
elif [[ $_comp_cword -eq 2 && "$_comp_prev" =~ ^(enter|delete|start|see|show|use)$ ]]; then
