...
if [[ "$_devsee_squash" == true ]]; then
    ...
    _dev_ssh_cmd "$_devsee_name" \
        "cd /workspace && git add -A; git reset HEAD -- .pr .issue .pnpm-store 2>/dev/null; git diff --cached --quiet || git commit -m 'WIP sync'; git push -f origin HEAD:refs/heads/${_devsee_branch}"
fi
